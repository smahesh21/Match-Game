...
class MatchGame extends Component {
  ...
  generateRandomImage = () => {
    const {clickedImagesList, score, timer} = this.state

    const updatedIndex = Math.floor(Math.random() * (imagesList.length - 1))
    this.setState({index: updatedIndex})
    const isIndexFound = clickedImagesList.some(index => index === updatedIndex)
    if (!isIndexFound) {
      this.setState(prevState => ({
        clickedImagesList: [...prevState.clickedImagesList, updatedIndex],
      }))
      this.setState({score: clickedImagesList.length})
    } else if (timer === 0) {
      this.setState({isGameStart: false})
    } else {
      this.setState({
  ...
  renderMatchGamePage = () => {
    const {activeTabId, index} = this.state
          ...
          <ul className="tabs-list">
            {tabsList.map(eachTab => (
              <TabsList
                eachTab={eachTab}
                key={eachTab.tabId}
                selectedTabItem={this.selectedTabItem}
    ...
    return (
      <div className="main-scorecard-container">
        <div className="scorecard-container">
          ...
          <h1 className="scorecard-score">{highestScore}</h1>
  ...
  render() {
    ...
    return (
      <div className="match-game-container">
        {this.renderHeader()}
        {!isGameStart ? this.renderScoreCard() : this.renderMatchGamePage()}
      </div>
    )
  }
}
